...
### Fanning out to more than one backend at once

`otelCollector.backends` (a list, distinct from the single-endpoint
`otelCollector.backendEndpoint` above) renders one `otlp/<name>` exporter per
entry, each with its own `sending_queue` and `retry_on_failure`, alongside
the `debug` exporter and alongside `otlp/backend` if that scalar is also
set — **both render if both are set**, so the one-key swap procedure above
stays true even once `backends` is in use. Each entry is:
...
otelCollector:
  backends:
    - name: candidate-a
      ...
      insecure: true          # omit for TLS
      headers:                # optional; ${env:...} expressions ONLY, never a literal
        authorization: "${env:CANDIDATE_A_AUTH_HEADER}"
      numConsumers: 8          # optional, defaults to 4
      queueSize: 10000         # optional, defaults to 5000
```

A credential referenced by `headers` never enters this repository: it comes
from a Vault-backed `ExternalSecret` in
`platform-gitops/infra-components/observability/otel-collector/` (the
pattern `infra-components/observability/secrets/loki-minio-externalsecret.yaml`
already uses) surfaced to the collector pod via the chart's `extraEnvFrom`.

This is the mechanism `mctlhq/mctl-gitops#1280` (issue #903's trace-backend
comparison) uses to send the same span stream to several candidates at
once — see `tests/fixtures/otel-eval-candidates.example-values.yaml` for a
worked two-candidate overlay and
`docs/adr/0001-agent-execution-trace-backend.md` for the comparison this
exists to run.
...
the committed representative DevLoop trace fixtures
(`tests/fixtures/devloop-trace.json`, `devloop-trace-redaction.json`, mounted
into its pod via the `otel-trace-fixture` ConfigMap generated by
`scripts/materialize-otel-trace-fixture.py`) to
`otel-collector.monitoring.svc.cluster.local:4318/v1/traces`, with fresh
trace and span ids per execution. A registered template runs nothing until
