...
SECRET_RE = re.compile(
    r"(ghp_[A-Za-z0-9]{36}|gho_[A-Za-z0-9]{36}|AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----|eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,})"
)
    ...
    for path in sorted(tenant_bindings.glob("*.yaml")) if tenant_bindings.exists() else []:
        ...
        for skill_name in binding.get("enabledSkills") or []:
            ...
            if not meta:
                ...
                continue
            if meta.get("visibility") in {"admin", "platform-internal"}:
