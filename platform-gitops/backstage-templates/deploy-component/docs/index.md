...
First-time setup for a new service — clones the repo, builds the Docker image in-cluster via Kaniko, commits GitOps config, and deploys to your team's namespace.
...
| Field | Description |
|---|---|
| Repo PAT | Fine-grained PAT with `Contents: Read` scope. Only needed if the GitHub App is not installed on the repository. |
