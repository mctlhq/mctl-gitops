...
**CVD source-agreement rule (Phase 2 Hook A) — exhaustive enum:**

`cvd_24h.agreement` всегда обязан присутствовать в snapshot output. Правила (применять в порядке, первое совпадение выигрывает):

1. `binance == null` (unreachable) AND `okx != null` → `agreement = "okx_only"`
2. `okx == null` AND `binance != null` → `agreement = "binance_only"`
3. `binance == null` AND `okx == null` → `cvd_24h = null` целиком, `missing_data: ["cvd_unreachable"]`, дальше скип секции
4. `binance == 0` AND `okx == 0` → `agreement = "both_zero"`, `direction = "balanced"`
5. `binance == 0` XOR `okx == 0` (ровно одна сторона ноль, другая non-zero) → `agreement = "partial_zero"`, направление от non-zero стороны (`direction = sign(non_zero) > 0 ? "buyers_dominant" : "sellers_dominant"`)
6. `sign(binance) == sign(okx)` (оба non-zero, знаки совпадают) → `agreement = "aligned"`, увереннее CVD-сигнал
7. `sign(binance) != sign(okx)` (оба non-zero, противоположные знаки) → `agreement = "split"` И append `risks: ["cvd source split: binance <sign>$<X>M, okx <sign>$<Y>M — directional pressure не подтверждается между биржами"]`

Каскад покрывает всё пространство `(binance ∈ {null, <0, 0, >0}) × (okx ∈ {null, <0, 0, >0}) = 16` сочетаний. Score-компоненты CVD остаются на агрегированном `net_usd`, но split/zero-warnings дают оператору сигнал что между биржами происходит арбитраж/decoupling/пустой период.
...
14. **CVD aggregation (Phase 2 Hook A):** `cvd_24h.net_usd = (binance_24h or 0) + (okx_24h or 0)`. Build `cvd_24h.per_exchange = { binance: <usd or null>, okx: <usd or null> }`. Determine `cvd_24h.agreement` через 7-правильный каскад из `### Local indicators` (CVD source-agreement rule выше — covers null, zero, aligned, split). На `"split"` — append `risks: ["cvd source split: binance <sign>$<X>M, okx <sign>$<Y>M"]`. На `"both_zero"` — append `risks: ["cvd both zero: ни одна биржа не показала чистого taker-имбаланса за 24h"]` (редкое для liquid pairs, но возможно для тонких символов в спокойные часы). Source string: `"binance_taker_buy_24h+okx_rubik_taker_volume_24h"`.

Если Bybit/Binance unreachable → append `bybit_kline_unreachable` / `binance_kline_unreachable` к `missing_data`, индикаторы помечаются `null`. Если **только Binance** unreachable И OKX все три endpoint-а отвечают → `cvd_24h.per_exchange.binance = null`, `agreement = "okx_only"`, OKX delta считается через OKX-родной close (не зависит от Binance). Если OKX любой из трёх endpoints unreachable → `okx_taker_volume_unreachable` в `missing_data`, `cvd_24h.per_exchange.okx = null`, `agreement = "binance_only"` — pipeline продолжается, CVD фоллбэчится на single-exchange Binance (как в Phase 1.6). Если оба источника недоступны → `cvd_24h = null`, `missing_data: ["cvd_unreachable"]`. Score деградирует gracefully на любом из источников. CoinGlass `get_futures_rsi_list` / `get_futures_macd_list` (steps 8-9) — secondary; используются для cross-check warning, не для scoring.
...
```json
{
  ...
  "cvd_24h": {
    ...
    "agreement": "aligned | split | partial_zero | both_zero | binance_only | okx_only",
