...
- **OKX V5 taker-volume (CVD leg #2, Phase 2 Hook A):** `GET https://www.okx.com/api/v5/rubik/stat/taker-volume-contract?instId=<COIN>-USDT-SWAP&period=4H`
  ...
  - **Per-bar pricing rule (важно):** для каждого OKX бара брать **свою** 4h close-цену с матчингом по timestamp с Binance kline (step 12 уже отдаёт 6 баров `[open_ts, ..., close, ...]` для того же 24h окна, оба источника UTC-aligned 4h). Per-bar delta_usd = `(buyVol − sellVol) * ctVal * close_at_that_bar`. Не использовать одну "последнюю" close-цену для всех 6 баров — в mixed-flow окне с дрифтом цены это может flip-нуть знак финального `cvd_24h.net_usd`.
  - Если кол-во баров OKX и Binance не совпадает (rubik отдал меньше 6 свежих баров или timestamps смещены): сматчить по `floor(ts / 14_400_000) * 14_400_000`, взять только пары с обоих источников; недостающие OKX-бары пропустить (накопится меньшая, но честная сумма). Сумма по сматченным парам = 24h OKX CVD.
...
13. **CVD 24h leg #2 — OKX (Phase 2 Hook A)** для same coin → два параллельных GET:
    ...
    - **Per-bar pricing:** для каждого OKX бара взять matching Binance close по timestamp (оба источника UTC-aligned 4h, step 12 уже отдал 6 баров `[ts, ..., close, ...]`). Per-bar delta_usd = `(buyVol − sellVol) * ctVal * close_at_that_bar`. Сматчить по `bucket_ts = floor(ts / 14_400_000) * 14_400_000`; пары без матча в Binance — пропустить (см. detailed pricing rule в `### Local indicators` секции выше). Sum сматченных пар = 24h OKX CVD.
