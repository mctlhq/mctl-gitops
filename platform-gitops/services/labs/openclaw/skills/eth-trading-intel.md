...
13. **CVD 24h leg #2 — OKX (Phase 2 Hook A)** для same coin → три параллельных GET:
    ...
    - **Per-bar pricing:** для каждого OKX taker-volume бара взять matching close из **OKX market/candles** по timestamp (`bucket_ts = floor(ts / 14_400_000) * 14_400_000`). Per-bar delta_usd = `(buyVol − sellVol) * ctVal * okx_close_at_that_bar`. Sum сматченных пар = 24h OKX CVD.
    - **Cross-venue price sanity (опционально, опираясь на step 12):** если Binance close доступен на том же ts и расходится с OKX close >2% → append `risks: ["okx/binance price spread >2% at <ts>: $<okx>, $<binance>"]`. Это сигнал dislocation/stale data. Сам OKX delta всё равно считается через OKX close (consistent self-source).
...
- **Не использовать** `interval` менее `4h` (1m, 5m, 15m, 30m, 1h) **в CoinGlass MCP-вызовах** — на HOBBYIST 403. Local Bybit kline на любом TF разрешён (Phase 1.6 — 15m/1h/4h/12h/1d через api.bybit.com).
...
- **Не вызывать** `get_futures_rsi_list` / `get_futures_macd_list` в default pipeline `/eth` / `/btc` / `/sol` — они tier-нестабильны на HOBBYIST, local Bybit = source of truth. Вызывать **только** из `/rsi` и `/macd` команд.
- **CVD source attribution обязателен.** На Phase 2 Hook B — `cvd_24h.source` склеивается из usable legs (joined по `+`) и `cvd_24h.per_exchange = {binance, okx, bybit, bitget}` всегда присутствует в snapshot (значения могут быть `null` при unreachable, либо leg может быть исключён через `<venue>_window_incomplete` в `missing_data` на cold-start). На full-stack happy path: `"binance_taker_buy_24h+okx_rubik_taker_volume_24h+bybit_publictrade_ws+bitget_publictrade_ws"`, ~95% global perp ETH. **Никогда** не возвращаться к 2-venue ("Binance + OKX, ~75%") как к baseline — Hook B сделал 4 venues статусом по умолчанию; деградация до 2 venues — fallback path, не норма, и обязана сопровождаться `<venue>_unreachable` / `<venue>_window_incomplete` в `missing_data` чтобы было видно почему.
