...
5. Output (default `all`):
   ...
   ```
   Buckets: `<30` extreme oversold, `30–40` oversold, `40–60` neutral, `60–70` bullish, `>70` extreme overbought.
...
4. Output:
   ```
   ETH MACD (12, 26, 9) on 4h (source: bybit kline)
   MACD:    +21.60
   Signal:  +18.98
   Hist:    +2.62  (rising — last 3 bars +1.84, +2.31, +2.62)
   Cross:   bullish (MACD > Signal, last cross-up ≈ 18 bars ago)
   ```
...
3. Output:
   ```
   ETH Bollinger Bands (20, 2σ) on 4h (source: bybit kline)
   Upper:    $2,413.72
   Mid:      $2,362.36
   Lower:    $2,311.00
   Price:    $2,408.69
   %B:       0.951  (price near upper band — overheated)
   Width:    4.35%  (normal vol regime)
   ```
   Squeeze warning если `width < 2.5%`: добавить строку `Squeeze: ⚠️ width <2.5% — break-out likely soon`.
...
3. Output:
   ```
   ETH ADX (14) on 4h (source: bybit kline)
   ADX:      32.6  (strong trend)
   +DI:      26.3
   -DI:      9.0
   Bias:     +DI > -DI by 17.3 → directional move is up
   ```
   Buckets: `<20` weak/no trend, `20–40` moderate, `>40` strong.
...
4. Output (sorted ascending by RSI — oversold first):
   ...
   SYMBOL    RSI    Bucket
   SOL       28.4   extreme oversold ⚠️
   BTC       45.2   neutral
   ETH       66.0   bullish
   ARB       72.5   extreme overbought ⚠️
   ```
