...
**Язык по умолчанию — русский.** Все snapshot/risk/scan/single-dimension outputs рендерятся на русском (operator chats — русскоязычные, cron-алерты тоже на русском с PR #125; чат-skill теперь синхронизирован). На английском — только если пользователь явно написал запрос на английском.
...
**РАЗРЕШЕНО** (русский, default): "наблюдение за лонг-сетапом", "наблюдение за шорт-сетапом", "возможный сквиз", "требуется ручной разбор", "ждать ретест", "не догонять", "преимущества нет", "ждать подтверждение", "крупняк/смарт мани на стороне лонга/шорта", "флэт без направления".
...
2. Если `null` — `Сигналов в памяти нет. Сначала запусти /eth, /btc, /sol или /scan.`
