...
1. **RESOLVED — "miniclaw" evaluation.** Operator research (2026-07-12) found no credible project named "miniclaw" that competes with or could replace openclaw as mctl-openclaw's fork base. "MiniClaw"/"mini-claw" is an overloaded, unclaimed name reused by many small, unrelated hobby projects (Telegram bots, edge-AI toys, MCP micro-kernels); `miniclaw.dev` itself reads as placeholder/template marketing copy (future-dated copyright, placeholder GitHub usernames, no version history). The one concrete, actively maintained project surfaced during research — `memovai/mimiclaw` (5.6k stars, MIT, active) — is a from-scratch C reimplementation of the openclaw agent loop for a $5 ESP32-S3 microcontroller: no Linux, no server process, single-user, no OAuth/gateway/multi-tenant surface. It is architecturally incompatible with mctl-openclaw's use case (multi-tenant server-side gateway with OAuth, Codex connect, k8s deployment) and is not a viable migration target. **Implementer action for task 2 / the evaluation deliverable: skip further research and post this conclusion verbatim (or a summary of it) as the evaluation comment on issue #38 — recommendation is to continue on openclaw, no migration issue needed.**
