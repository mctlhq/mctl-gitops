# Design: incident-a58acb7b

## Diagnosis
The pod `mctl-agents-issue-poll-1785258000-notify` (an Argo Workflows workflow job) is stuck in ContainerWaiting state for over 1 hour. Absence of logs indicates the pod never reached Running state. Root cause: the workflow job container is likely waiting on one of: image pull (slow/failed pull), pending resource quota (CPU/memory unavailable), or node capacity exhaustion. The Argo Workflows controller may have insufficient pod scheduling slots or the cluster is under heavy resource pressure. Since this is a dynamic workflow job name, the pod likely ran out of scheduled time or hit a resource backlog.
