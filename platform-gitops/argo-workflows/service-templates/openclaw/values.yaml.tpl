    ...
    args:
      - |
        ...
        async function check() {
          ...
          let failureCount = creds.refreshFailureCount || 0;
          // A reconnect from the Control UI (or a refresh performed by
          // mctl-mcp-proxy) writes a brand new refresh token into this file
          // but leaves refreshFailureCount untouched, while bumping updatedAt
          // — which pushed the cooldown below an hour further out instead of
          // ending it. Observed live 2026-08-23: the UI read "Connected as
          // mashkovd" while this loop kept logging "skipping: 20 consecutive
          // failures". A counter recorded against a token that is no longer
          // the current one is stale, so drop it.
          // A counter written before refreshFailureToken existed carries no
          // marker at all. That is exactly the state this change is meant to
          // repair — admins-openclaw sat at count 20 with no marker — so a
          // missing marker must clear too, not just a mismatched one. If the
          // token really is still dead the next attempt simply re-arms the
          // counter, this time with a marker.
          if (failureCount > 0 && creds.refreshFailureToken !== creds.refreshToken) {
            const why = creds.refreshFailureToken ? 'credentials replaced' : 'no token marker';
            process.stdout.write('[mctl-refresh] ' + new Date().toISOString() + ' clearing ' + failureCount + ' stale failures (' + why + ')\n');
            // Merge the cleared fields into the freshest file rather than the
            // snapshot read at the top of check(): mctl-mcp-proxy or a Control
            // UI reconnect may have rotated the tokens in between, and writing
            // the snapshot back would restore an already-rotated refresh token
            // — killing the very credentials this is trying to unblock. Same
            // re-read discipline the failure path below already uses.
            let latest; try { latest = JSON.parse(fs.readFileSync(CREDS, 'utf8')); } catch { latest = null; }
            creds = { ...(latest || creds), refreshFailureCount: 0, refreshFailureFirstAt: null, refreshFailureToken: null };
            fs.writeFileSync(CREDS + '.tmp', JSON.stringify(creds, null, 2), { mode: 0o600 }); fs.renameSync(CREDS + '.tmp', CREDS);
            failureCount = 0;
          }
          if (failureCount > 0) {
          ...
          if (!ok || body.error) {
            ...
            if (invalid) {
              ...
              const failed = { ...creds, refreshFailureCount: (creds.refreshFailureCount || 0) + 1, refreshFailureFirstAt: creds.refreshFailureFirstAt || new Date().toISOString(), refreshFailureToken: creds.refreshToken, updatedAt: new Date().toISOString() };
          ...
          const updated = { ...creds, accessToken: body.access_token, refreshToken: body.refresh_token || creds.refreshToken, expiresAt: new Date(now + expiresIn * 1000).toISOString(), updatedAt: new Date(now).toISOString(), refreshFailureCount: 0, refreshFailureFirstAt: null, refreshFailureToken: null };
