    ...
    args:
      - |
        ...
        if [ ! -f "$WORKSPACE/AGENTS.md" ]; then
          echo "Fresh workspace — seeding MCTL agent identity files only. Skills are tenant-managed."
        ...
        # Stamp .layer2 only on the skill names the setup init owns.
        # This list covers both the legacy base64 seed set and the
        # /app/mctl-skills image overlay (which currently ships only
        # mctl-platform). Names that don't have a matching dir — e.g.
        # after the base64 seeds are dropped from this values file —
        # are silently skipped. S3-restored Layer-3 dirs stay unmarked
        # so the fan-out sidecar picks them up on its next reconcile.
        for name in mctl-platform mctl-agent-external mctl-gitops-remediation mctl-github-remediation; do
...
extraContainers:
  ...
  # Layer-3 skills fan-out: watch the tenant-scoped <team>-openclaw-skills
    ...
    args:
      - |
        ...
        while true; do
          ...
          # Layer-3 skill whenever ArgoCD blinks.
...
extraVolumes:
  ...
  - name: layer3-skills-cm
    configMap:
      name: __TEAM_NAME__-openclaw-skills
      optional: true
  - name: openclaw-scripts
    configMap:
      name: openclaw-scripts
