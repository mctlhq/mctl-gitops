  ...
  - name: restore-state
    ...
    args:
      - |
        ...
        NEW_STATE_MARKER=""
        LEGACY_STATE_MARKER=""
        if mc ls --recursive s3/platform-state/__TEAM_NAME__/__SERVICE_NAME__/ 2>/dev/null | head -1 | grep -q .; then
          NEW_STATE_MARKER=1
        fi
        if mc ls --recursive s3/platform-state/__SERVICE_NAME__/__TEAM_NAME__/ 2>/dev/null | head -1 | grep -q .; then
