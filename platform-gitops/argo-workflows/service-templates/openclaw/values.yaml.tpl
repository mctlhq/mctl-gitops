    ...
    args:
      - |
        ...
        async function check() {
          ...
          if (!creds.refreshToken || !creds.clientId || (creds.refreshFailureCount || 0) > 0) return;
          ...
          if (!ok || body.error) {
            ...
            if (invalid) {
              ...
              fs.writeFileSync(CREDS + '.tmp', JSON.stringify(failed, null, 2)); fs.renameSync(CREDS + '.tmp', CREDS);
            }
            return;
          }
          const now = Date.now();
          ...
          fs.writeFileSync(CREDS + '.tmp', JSON.stringify(updated, null, 2)); fs.renameSync(CREDS + '.tmp', CREDS);
